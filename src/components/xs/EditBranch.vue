<template>
    <v-content>
        ...
        <!-- Edit Branch -->
        <v-layout row wrap style="maring-top:50px;">
            <v-flex xs1></v-flex>
            <v-flex class="payload" xs10>
                <!-- add currency -->
                <v-layout row wrap>
                ...
                <div v-if="loaded">
                  <v-layout  row wrap v-for="(x, index) in rate" v-bind:key="x.index" style="border:solid 1px; margin-bottom:10px; background-color:#F0F0F0;">
                      <v-flex xs10  >
...
import DynamicCurrency from '@/components/xs/DynamicCurrency'
