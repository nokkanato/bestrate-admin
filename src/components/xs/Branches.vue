<template>
    <v-content style="margin-top:0px;">
        ...
        <v-layout row wrap style="margin-top:10px;">
            <v-flex xs6>
                <div @click="toggleDialog" class="button-add-branchess">
        ...
        <v-layout row wrap>
            <v-flex xs12>
                <v-layout row wrap>
                <v-flex  xs12 v-for="(x, index) in branches" v-bind:key="index">
                    <div class="element-branchh-xs">
                        <!--  -->
                        <p class="element-header">{{x.name}}</p>
...
.button-add-branchess:hover {
    background-color: #264089;
    color:white;
    border: #264089 solid 1px;
    cursor: pointer;
}
.button-add-branchess {
  transition:  0.4s;
  margin-left:10px;
  font-size:15px;
  font-weight: 600;
